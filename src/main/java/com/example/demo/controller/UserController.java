...
@RestController
@CrossOrigin(origins = "*") // WAJIB agar AJAX dari browser port lain bisa masuk
public class UserController {
    ...
    // PERBAIKAN PUT: Tambahkan /{id} di path
    @PutMapping(
            path = "/api/users/{id}",
            consumes = MediaType.APPLICATION_JSON_VALUE,
            produces = MediaType.APPLICATION_JSON_VALUE
    ...
    // PERBAIKAN DELETE: Tambahkan /{id} di path
    @DeleteMapping(
            path = "/api/users/{id}",
            produces = MediaType.APPLICATION_JSON_VALUE
    )
    public ResponseEntity<Map<String, Object>> DeleteUser(@PathVariable("id") String id) {
        ...
        return ResponseEntity.status(HttpStatus.OK).body(Map.of(
                "status", "success delete with id " + id
        ));
    }
}